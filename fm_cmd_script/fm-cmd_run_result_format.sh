...
function func_main
{
    ...
    #done
    if [ ${remaining_args[0]} = lsusb ];then
        lsusb
        echo "总线编号 总线上的编号 厂商ID:产品ID 描述:"
    elif [ ${remaining_args[0]} = lsmod ];then
        lsmod
        echo "模块名称         占用内存大小 被使用计数  哪些模块或进程在使用它"
    else
        echo "ERROR:Unknow cmd:${remaining_args[0]}"
        echo "only support "
        return 1
    fi
    return 0
