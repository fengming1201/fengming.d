...
function man_page_useinfo
{
    echo "章节号	  内容说明"
    echo ""
    echo "1          用户命令（可执行程序，如 ls, cp）"
    echo "2          系统调用（如 open, read, write, fork）	"
    echo "3          C 库函数（如 printf, malloc, pthread_mutex_init）"
    echo "4          特殊文件（通常是 /dev 下的设备文件）"
    echo "5          文件格式与约定（如 passwd, fstab, crontab）"
    echo "6          游戏与趣味程序"
    echo "7          杂项（如 man(7), regex(7), signal(7)）"
    echo "8          系统管理命令（仅超级用户使用，如 mount, ifconfig）"
    echo "9          内核开发文档（某些系统支持，主要是内核 API）"
    return 0
}
...
# Example:
##
function func_man_page_install
{
    ...
    #done
    local man_page=("manpages-dev" "manpages-posix-dev")

    ${maybeSUDO} apt install -y ${man_page[@]}
